namespace Blog.Data
{
    ...
    internal static class EfExpressionHelper
    {
        ...
        private static readonly MethodInfo ValueBufferGetValueMethod =
            typeof(ValueBuffer).GetRuntimeProperties().Single(p => p.GetIndexParameters().Any()).GetMethod ?? throw new Exception();

        private static readonly MethodInfo EfPropertyMethod =
            typeof(EF).GetTypeInfo().GetDeclaredMethod(nameof(Property)) ?? throw new Exception();
            ...
            where TEntity : class
        {
            ...
            var keyProperties = dbContext.Model.FindEntityType(entityType)?.FindPrimaryKey()?.Properties ?? throw new Exception();
            ...
            ParameterExpression entityParameter)
        {
            ...
            return predicate ?? throw new Exception();
        }
    }
}
